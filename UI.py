...
import time
import json
# from joystick_visual import visuals
...
client.tls_set(cert_reqs=mqtt.ssl.PROTOCOL_TLS)
...
while not exit: 
    ...
    try:
        if emergency is False: # if emergency is false we blit the normal emergency button
            ...
            # arrowupImage = pygame.image.load(visuals.arrow_up).convert_alpha()
            if lock is False: # if lock is false we blit the unlocked lock
                locking = screen.blit(pygame.image.load('images/locked.png'), (1025,360))
                # print(position , " has unlocked")
                if movement == 'forward':
                    screen.blit(pygame.image.load('images/locked.png'), 1025,360)
                    print( " blitted to position ")
